...
geolocator = Nominatim(user_agent="vedic_astro_app_final")

# Constants - Added Outer Planets
AYANAMSA_MAP = {"Lahiri": 1, "Raman": 3, "KP": 5, "Pushya": 41}
# Swiss Eph IDs: Pluto=10, Neptune=9, Uranus=8
PLANETS_MAP = {
...
def draw_north_indian_chart(house_data, lagna_rashi, person_name):
    ...
    for i in range(12):
        ...
        ax.text(centers[i][0], centers[i][1]-40, str(r_num), color='red', fontsize=12, ha='center')
        p_list = "\n".join(house_data[i])
        ax.text(centers[i][0], centers[i][1], p_list, fontsize=9, ha='center', va='center', fontweight='bold')
...
with st.sidebar:
    ...
    p_name = st.text_input("Name", "User Chart")
...
if st.button("Generate Professional Patrika"):
    try:
        # High Precision Time Parsing
        try:
            tob_parsed = datetime.strptime(tob_str, "%H:%M:%S")
        except:
            tob_parsed = datetime.strptime(tob_str, "%H:%M") # Fallback to HH:MM
        ...
        # UI Display
        st.success(f"Calculated with High Precision for {city}")
